...
- Optional: the [`pack` CLI](https://buildpacks.io/docs/install-pack/) and [`syft`](https://github.com/anchore/syft#installation) for buildpack metadata and SBOM inspection (see [Comparing the two images](#comparing-the-two-images)). `jq` is also handy for filtering SBOM JSON.
...
The tool-supported way to compare what is installed in each image is its Software Bill of Materials. Both images embed an SBOM (CycloneDX, SPDX, and Syft formats) under `/layers/sbom/`, generated by the Paketo `syft` buildpack. Two standard tools read it: [`syft`](https://github.com/anchore/syft) prints or exports an inventory, and the Cloud Native Buildpacks [`pack`](https://buildpacks.io/docs/install-pack/) CLI extracts the copy already attached to the image.

Print a human-readable inventory of each image (operating-system packages plus Java archives):

```bash
syft hello-world:0.0.1-SNAPSHOT-jvm    -o table
syft hello-world:0.0.1-SNAPSHOT-native -o table
```

`syft` reads the image from the local Podman/Docker daemon, honoring `DOCKER_HOST` (see [Podman socket for `bootBuildImage`](#podman-socket-for-bootbuildimage)). If the daemon backend is unavailable, point it at an exported archive instead:

```bash
podman save hello-world:0.0.1-SNAPSHOT-jvm --format oci-archive -o jvm.oci
syft oci-archive:jvm.oci -o table
```

Diff the two dependency sets by name and version using the CycloneDX output and `jq`:

```bash
syft hello-world:0.0.1-SNAPSHOT-jvm    -o cyclonedx-json > jvm.cdx.json
syft hello-world:0.0.1-SNAPSHOT-native -o cyclonedx-json > native.cdx.json

diff \
  <(jq -r '.components[] | "\(.name) \(.version)"' jvm.cdx.json    | sort -u) \
  <(jq -r '.components[] | "\(.name) \(.version)"' native.cdx.json | sort -u)
```

The Cloud Native Buildpacks equivalent extracts the SBOM the build already attached (one CycloneDX document per contributing buildpack):

```bash
pack sbom download hello-world:0.0.1-SNAPSHOT-jvm    --output-dir ./sbom-jvm
pack sbom download hello-world:0.0.1-SNAPSHOT-native --output-dir ./sbom-native
```

If you would rather not install `syft` or `pack`, the repository includes `scripts/compare-image-sboms.sh` as a fallback. It reads the same embedded SBOMs (and the base image's package database) directly from the built images using only `podman` and `jq`, and prints the per-image inventories and their differences:

```bash
./scripts/compare-image-sboms.sh
# or compare any two images:
./scripts/compare-image-sboms.sh hello-world:0.0.1-SNAPSHOT-jvm hello-world:0.0.1-SNAPSHOT-native
```

Expect an identical set of operating-system packages (both variants share the same Noble `run-tiny` base) and an identical set of Java dependencies. The difference is the runtime the buildpacks contribute: the JVM image adds the BellSoft Liberica JRE and the Spring Boot and Spring Cloud Bindings runtime helpers, while the native image contains none of them because that runtime is compiled into the executable. That contributed JRE layer accounts for most of the JVM image's larger size.
